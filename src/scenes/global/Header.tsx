...
const navigation = [
  { name: 'Product', href: '/calories_calculator/FoodSearch' },
  { name: 'Recipe', href: '/calories_calculator/RecipeSearch' },
  { name: 'Nutrition', href: '/calories_calculator/NutritionSearch' },
]
  ...
  return (
        ...
        <nav className="mx-auto flex max-w-5xl items-center justify-between p-10 lg:px-8" aria-label="Global">
          ...
          <div className="hidden lg:flex lg:gap-x-12">
            {navigation.map((item) => (
              <Link key={item.name} to={item.href} className="text-lg font-semibold leading-6 text-gray-900" onClick={() => setMobileMenuOpen(false)}>
                {item.name}
              </Link>
            ))}
          </div>
